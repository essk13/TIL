...
def water_charge():
    water_charge = list(map(int, input().split()))
    A_company = water_charge[0] * water_charge[-1]
    B_company = 0

    if water_charge[-1] > water_charge[2]:
        B_company = water_charge[1] + water_charge[3] * (water_charge[-1] - water_charge[2])
    else:
        B_company = water_charge[1]
